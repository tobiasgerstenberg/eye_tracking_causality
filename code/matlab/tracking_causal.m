...
try
    ...
    % GIVE INSTRUCTIONS
    ShowCursor;
    instrimage = imread('instructions1.jpg','jpg');
    Screen('PutImage',win,instrimage,[x0-500,y0-375,x0+500,y0+375]);
    ...
    for     k = 1:length(randmovies);       
        ...
        for iteration= 1:2
            ...
            if iteration == 1
                ...
                ShowCursor; %Show mouse cursor
                if outcomes(i) == 0
                    outcomeText = 'Ball          prevented ball          from going through the gate.';
                    xposA = 210;
                    xposB = 485;
                else
                    outcomeText = 'Ball          caused ball          to go through the gate.';
                    xposA = 270;
                    xposB = 505;
                end
                
                %draw balls
                ballA = imread('ballA.jpg','jpg');
                ballB = imread('ballB.jpg','jpg');
                ypos = 305;
                Screen('PutImage',win,ballA,[xposA-30,ypos-30,xposA+31,ypos+31]);
                Screen('FrameRect', win, [128 128 128], [xposA-30,ypos-30,xposA+31,ypos+31],1);
                
                Screen('PutImage',win,ballB,[xposB-30,ypos-30,xposB+31,ypos+31]);
                Screen('FrameRect', win, [128 128 128], [xposB-30,ypos-30,xposB+31,ypos+31],1);
                
                DrawFormattedText(win, 'How much do you agree with the following statement?', 'center', y0-200, [0 0 0]);
                DrawFormattedText(win, outcomeText, 'center', y0-100, [0 0 0]);
                Screen('FrameRect', win, [0 0 0], [x0-300 y0-20 x0+300 y0+20],3);
                
                Screen('TextSize',win,20);
                DrawFormattedText(win, 'not at all', x0-340, y0+30, [0 0 0]);
                DrawFormattedText(win, 'very much', x0+250, y0+30, [0 0 0]);
